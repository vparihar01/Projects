...
begin
  ...
  mkdir(File.join(Rails.root.to_s, "app/views/mini_captcha")) unless File.exist?(File.join(Rails.root.to_s, "app/views/mini_captcha"))
  FileUtils.cp_r(
    File.join(File.dirname(__FILE__), "installables/views/_mini_captcha.html.erb"),
    File.join(Rails.root.to_s, "app/views/mini_captcha/_mini_captcha." + (Rails::VERSION::STRING < '1.8' ? 'rhtml' : 'html.erb'))
  )
  puts "View created."

  puts "Copying stylesheet..."
  mkdir(File.join(Rails.root.to_s, "public/stylesheets")) unless File.exist?(File.join(Rails.root.to_s, "public/stylesheets"))
  FileUtils.cp_r(
    File.join(File.dirname(__FILE__), "installables/public/stylesheets/_mini_captcha.css"),
    File.join(Rails.root.to_s, "public/stylesheets/_mini_captcha.css")
  )
