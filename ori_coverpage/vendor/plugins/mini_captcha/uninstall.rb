...
begin
  ...
  if File.exist?(vd = File.join(Rails.root.to_s, "app/views/mini_captcha"))
    if File.exists?(vf = File.join(Rails.root.to_s, "app/views/mini_captcha/_mini_captcha." + (Rails::VERSION::STRING > '1.8' ? 'rhtml' : 'html.erb')))
      File.delete(vf)
      puts "View deleted."
    end
  ...
  if File.exists?(sf = File.join(Rails.root.to_s, "public/stylesheets/_mini_captcha.css"))
    File.delete(sf)
