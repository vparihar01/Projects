...
module ActiveRecord
  module Validations
    module ClassMethods
      ...
      def validate_http_url_with_redirection(configuration, record, field, host, port, path, redirects = nil, redirected = false)
        ...
        if response.kind_of?(Net::HTTPRedirection) && redirects
          if redirects > 0
            # TIM: begin -- save the last redirect
            record.redirect = response['location'] if record.respond_to?(:redirect)
            # TIM: end -- save the last redirect
            uri = URI.parse(response['location'])
            validate_http_url_with_redirection(configuration, record, field, uri.host || host, uri.port || port, uri.path, redirects - 1, true)
          else
          ...
          record.code = response.code if record.respond_to?(:code)
          record.meta_title = nil if record.respond_to?(:meta_title)
          record.meta_description = nil if record.respond_to?(:meta_description)
          record.redirect = nil if record.respond_to?(:redirect) && !redirected
          if configuration[:valid_responses].any? { |valid_response| response.kind_of? valid_response }
            if doc = Hpricot(response.body)
              if doc.at("title") && title = doc.at("title").inner_html
                record.meta_title = title.strip if record.respond_to?(:meta_title)
              end
              if doc.at('meta[@name="description"]') && description = doc.at('meta[@name="description"]')['content']
                record.meta_description = description.strip if record.respond_to?(:meta_description)
              end
            end
          else
            record.errors.add(field, error_msg)
          end
          # TIM: end -- set values
        end
        # TIM: begin -- return response
        response
        # TIM: end -- return response
      end
