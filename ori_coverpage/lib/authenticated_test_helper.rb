# create aliases in ActionController::TestCase
ActiveSupport::TestCase.class_eval do
  alias :assert_diff :assert_difference
  alias :assert_no_diff :assert_no_difference
end

module AuthenticatedTestHelper
  # Sets the current user in the session from the user fixtures.
  def login_as(user)
    @request.session[:user] = user ? users(user) : nil
  end
