...
namespace :db do
  desc "Setup the dev db"
  task :bootstrap => [ :remigrate, 'db:fixtures:load' ] do
    puts "=> You're all set!"
  end

  desc "Migrate down, then up, and load fixtures"
  task :remigrate => :environment do
    raise "Can't remigrate in this Rails.env!" unless %w(development test build staging).include? Rails.env
...
task :setup_sh_mysql do
  def sh_mysql(command = 'mysql', env = Rails.env)
    require 'yaml'
