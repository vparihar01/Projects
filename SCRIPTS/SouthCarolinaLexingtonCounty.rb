...
doc2 = scrape.post(BASE, post_args2)
 ...
 url=URI.encode("http://jail.lexingtonsheriff.net/p2c/InmateDetail.aspx?navid=#{id}")
doc5= scrape.get(url)

 arrest.image1=arrest.image2="http://jail.lexingtonsheriff.net/p2c/Mug.aspx"
 bondamt=doc5.css('#ctl00_ctl00_mainContent_CenterColumnContent_lblTotalBoundAmount').inner_html.gsub(',','')#.to_i


descr=doc5.css('#ctl00_ctl00_mainContent_CenterColumnContent_dgMainResults tr:nth-child(2) td:nth-child(1)').to_s.split('<td>').last.split('</td>').first rescue ""
