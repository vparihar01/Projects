...
BASE="http://p2c.nhcgov.com/p2c/#{extract_url}"
  dco = scrape.get(BASE)
name=dco.css('tr td p span#ctl00_ctl00_mainContent_CenterColumnContent_lblName.ShadowBoxFont').inner_html
charges=dco.xpath('//*[(@id = "ctl00_ctl00_mainContent_CenterColumnContent_dgMainResults")]//tr[(((count(preceding-sibling::*) + 1) = 2) and parent::*)]//td[(((count(preceding-sibling::*) + 1) = 1) and parent::*)]')[0].inner_html
...
post_args = {
 ...
 docu = scrape.post(DETAIL, post_args)
  next_link=docu.css('tr td')[56].css('a')
   next_link=docu.css('tr td')[56].css('a').size
  next_link=docu.css('tr td')[56].css('a')[i+1]['href'].split("'")[1] rescue ''
