=begin
    Erie PA Erie County.rb is a Ruby file/crawler which Scraps the Offender Details from Erie County
    URL => "http://www.pameganslaw.state.pa.us/SearchResults.aspx?CountyName=ERIE&amp;"!!!      
=end
...
STATE = "Erie"				# State Is mentioned
COUNTY = "Erie County"			# County Is mentioned
CITY = "Erie"					# City is mentioned
...
form["ctl00$SessionTimeout1$Timer"]="300 Minutes remaining."
 ...
 page=@agent.submit(form,form.buttons.last)  			# Submitting the form
temp_jar=@agent.cookie_jar							# Taking The cookie
@agent=Mechanize.new
@agent.cookie_jar=temp_jar							# Maintaining the Cookie
page=@agent.get("http://www.pameganslaw.state.pa.us/SearchResults.aspx?CountyName=ERIE&amp;") rescue "" 
...
count=no_of_offenders/10  
p count +=1 unless no_of_offenders%10==0  				# Increases by 1 if remainder is there
...
page2=@agent.post("http://www.pameganslaw.state.pa.us/SearchResults.aspx?CountyName=ERIE&amp%3b",next_page)  rescue ""			# Posting the params
p page2.parser.css("#ctl00_SessionTimeout1_Timer")[0]["value"]
page2.parser.css("tr.normal").each {|i|
links << i["onclick"].split("='").last.split("';").first 					# Taking the links
}	
...
p next_link.parser.css("#ctl00_SessionTimeout1_Timer")[0]["value"]  rescue ""
 p date=next_link.parser.css("#ctl00_MainContent_fvDetail_TableCell7").inner_html  rescue ""	# Extracting Date
p name=next_link.parser.css("#ctl00_MainContent_fvDetail_tblName").inner_html rescue ""  
...
desc_link=@agent.post("http://www.pameganslaw.state.pa.us/#{li}",post_args2)  				# Description is being scrapped here
 desc=desc_link.parser.css("#ctl00_MainContent_fvDetail_TableCell3").inner_html.split("-").last  
